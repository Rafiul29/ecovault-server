...
> {
            ...
            const searchConditions: Record<string, unknown>[] = searchableFields.map((field) => {
                if (field.includes(".")) {
                    ...
                    if (parts?.length === 2) {
                        ...
                        const nestedField = parts[2] as string
        ...
        Object.keys(filterParams).forEach((key) => {
            ...
            if (key.includes(".")) {
                ...
                if (parts.length === 2) {
                    ...
                    const queryNestedIs = (queryNestedRelation.is || {}) as Record<string, unknown>;
                    const countNestedIs = (countNestedRelation.is || {}) as Record<string, unknown>;

                    queryNestedIs[nestedField] = this.parseFilterValue(value);
                    countNestedIs[nestedField] = this.parseFilterValue(value);

                    queryNestedRelation.is = queryNestedIs;
                    countNestedRelation.is = countNestedIs;
    ...
    sort(): this {
        ...
        if (sortBy.includes(".")) {
            ...
            if (parts.length === 2) {
                ...
                const relation = parts[0] as string
                const nestedField = parts[1] as string
                const nestedRelation = parts[2] as string
    ...
    fields(): this {
        ...
        if (fieldsParam && typeof fieldsParam === 'string' && fieldsParam.trim() !== '') {
            ...
            if (fieldsArray.length > 0) {
                ...
                // Only set select if we have actual fields
                if (Object.keys(this.selectFields).length > 0) {
                    ...
                    // Clear include completely to prevent conflicts
                    this.query.include = undefined as any;
                }
            }
        }
        return this;
    ...
    async execute(): Promise<IQueryResult<T>> {
        // Build clean query object, excluding empty select/include
        ...
        // Only add select if it exists and is not empty
        if (this.query.select && typeof this.query.select === 'object' && Object.keys(this.query.select as Record<string, unknown>).length > 0) {
            cleanQuery.select = this.query.select;
        }

        // Only add include if it exists and is not empty, and select is not present
        if (this.query.include && typeof this.query.include === 'object' && Object.keys(this.query.include as Record<string, unknown>).length > 0 && !cleanQuery.select) {
            cleanQuery.include = this.query.include;
        }

        const [total, data] = await Promise.all([
            this.model.count(this.countQuery as Parameters<typeof this.model.count>[0]),
            this.model.findMany(cleanQuery as Parameters<typeof this.model.findMany>[0])
        ])
    ...
    private parseRangeFilter(value: Record<string, string | number>): PrismaNumberFilter | PrismaStringFilter | Record<string, unknown> {
        ...
        Object.keys(value).forEach((operator) => {
            const operatorValue = value[operator] as number | string;
